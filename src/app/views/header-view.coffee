  ...
  app.headerView = (ctrl) ->
    m 'header', [
      m 'nav', [
        ...
        m 'ul', [
          m 'li', m 'a[href=#/]', { className: if not ctrl.filter() then 'link--active' else '' }, '#all (' + app.storage.products().length + ')'
      ...
      m 'form', { onsubmit: ctrl.search.bind ctrl }, [
        ...
        m 'input[type="text"][placeholder="Search..."]', { oninput: m.withAttr('value', ctrl.queryInput), value: ctrl.queryInput() }
